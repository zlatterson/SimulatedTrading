...
class TestCallPut(unittest.TestCase):
    ...
    def test_refreshes_position_value(self):
        ...
        user_amazon_call = CallPutOptionService.make_position(amazon_call_contract,"BUY",3,self.user)
        print("__")
        print("original:", user_amazon_call.running_pl)
        amazon_call_contract.fetch_c_price()
        print("after:", user_amazon_call.running_pl)

    @unittest.skip("working")
    def test_real_example(self):
        original_money = self.user.money
        stock = StockService.make_stock("GOOGL")
        call_contract = CallPutContractService.make_contract("GOOGL220826C00050000",stock,"CALL")
        print("user starting money:", self.user.money)
        user_call = CallPutOptionService.make_position(call_contract,"BUY",3,self.user)
        print("user money after purchase:", self.user.money)
        print("first current_contracts_value:", user_call.current_contracts_value)
        print("first running_pl:", user_call.running_pl)
        print("first running_pl % :", user_call.running_pl_percentage)
        print("first contract value:", call_contract.current_contract_value)
        call_contract.fetch_c_price()
        sleep(20)
        print("second contract value:", call_contract.current_contract_value)
        print("second current_contracts_value:", user_call.current_contracts_value)
        print("second running_pl:", user_call.running_pl)
        print("second running_pl % :", user_call.running_pl_percentage)
    ...
    def test_no_error(self):
        ...
        user_call = CallPutOptionService.make_position(call_contract,"BUY",3,self.user)
        print("user money after purchase:", self.user.money)
        print("first value:", user_call.current_contracts_value)
        sleep(10)
        call_contract.fetch_c_price()
        print("second value:",user_call.current_contracts_value)
